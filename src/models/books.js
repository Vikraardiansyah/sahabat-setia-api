...
module.exports = {
    ...
    getBookByRecommended: function () {
        return new Promise(function (resolve, reject) {
            connection.query(`SELECT books.id, books.title, books.description, books.id_genre, genre.genre, books.id_author, author.author, books.id_status, status.status, books.image, books.email_borrow, books.updated_at, books.created_at FROM books INNER JOIN genre ON books.id_genre = genre.id INNER JOIN author ON books.id_author = author.id INNER JOIN status ON books.id_status = status.id WHERE books.recommended=1 ORDER BY books.created_at DESC`, function (error, result) {
                if (!error) {
                    resolve(result)
                } else {
                    reject(new Error(error))
